...
function Artifact() {
  ...
  const [activFile, setActivFile] = useState(0)
  const [copied, setCopied] = useState(false)
  const [mobileOpen, setMobileOpen] = useState(false)
  if (artifacts.length == 0) return;


  // Safely grab the title if the artifact exists, otherwise provide a fallback
  const displayTitle = artifacts?.[0]?.title || "Code Generation";

  const file = artifacts[0]?.files[activFile]
        ...
        {!collapsed ? <div className='flex flex-col h-full bg-[#0d0f14]'>
          <div className='h-14 px-4 border-b border-white/6 flex items-center gap-3 shrink-0'>
            ...
            border-indigo-500/20 shrink-0'>
              ...
              {/* Display the safe title with truncation in case it's long */}
              <div className='text-[13px] font-medium text-slate-200 truncate'>
                {displayTitle}
              </div>
            </div>
            <div className='flex items-center gap-1 shrink-0'>
              <button onClick={handleCopy} className='flex items-center gap-1.5 px-2.5 py-1.5 text-[11px] font-medium text-slate-400 hover:text-slate-200
            hover:bg-white/5 rounded-lg transition-colors duration-150 bg-transparent border-none cursor-pointer'
              >
          ...
          {tab === "code" && <div className='h-auto flex border-b border-white/6 overflow-x-auto scrollbar-none [&::-webkit-scrollbar]:hidden shrink-0'>
            {artifacts[0]?.files?.map((f, index) => (
              <button
                onClick={() => setActivFile(index)}
                key={index}
                className={`px-4 py-2.5 text-[11px] font-medium whitespace-nowrap transition-colors duration-150 border-r border-white/5 relative cursor-pointer bg-transparent text-indigo-400 
        ${activFile === index ? "text-indigo-400" : "text-slate-500 hover:text-slate-300"}
        `}
              >
                {f?.name}
                {activFile === index && <div className='absolute bottom-0 left-0 right-0 h-0.5 bg-indigo-500 rounded-t-full' />}
          ...
          <div className='flex-1 overflow-hidden'>
            ...
            >
              <iframe srcDoc={previewDoc} sandbox='allow-scripts' title='preview' className='w-full h-full bg-white' />
            </motion.div>
              :
              <motion.div
                initial={{ opacity: 0 }}
          ...
          <div className='hidden lg:flex border-l border-white/6 items-center py-4 gap-3 shrink-0 flex-col h-full bg-[#0d0f14]'>
            <button className='flex items-center justify-center w-7 h-7 rounded-lg text-slate-500 hover:text-slate-200 hover:bg-white/5 transition-colors duration-150 bg-transparent border-none cursor-pointer shrink-0'
  ...
  return (
    <>
      ...
      <button
        ...
        className='lg:hidden fixed bottom-24 right-4 z-40 flex items-center gap-2 px-3.5 py-2
    ...
    z-50 bg-black/60 backdrop-blur-sm'/>
          ...
          <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ duration: 0.25, ease: "easeInOut" }}
            className='lg:hidden fixed inset-y-0 right-0 z-50 w-[80vw] max-w-105
    border-l border-white/6 overflow-hidden'>
